...
Also exported for the print adapters: the vendored DM Sans font files (cleanbill/fonts/, OFL), `register_fonts()` for
...
# The return address printed on the envelope (Lob `from`) and in the letter's §41.0051 footer (SPEC-11 §5; Charlie's
# §7 step 2 — the registered address, a box, or home; returned mail comes back here). `name` is the entity line (O-06).
# The zip "00000" marks the placeholder: cleanbill.letters.batch refuses to send with a live key while it is set.
RETURN_ADDRESS = {"name": LEGAL_NAME, "line1": "RETURN ADDRESS NOT SET", "line2": "", "city": "Austin", "state": "TX", "zip": "00000"}


def return_address_is_placeholder(addr: dict | None = None) -> bool:
    a = addr or RETURN_ADDRESS
    return a.get("zip") == "00000" or not a.get("line1") or a.get("line1") == "RETURN ADDRESS NOT SET"
...
# ---- generated files ------------------------------------------------------------------------------------------------
HEADER = "GENERATED by `python -m cleanbill.brand --sync` from apps/web/src/styles/tokens.css + theme-{theme}.css — do not edit."
...
def sync(check: bool) -> int:
    ...
    if check:
        if stale:
            print("stale generated brand files (run `python -m cleanbill.brand --sync`):\n  " + "\n  ".join(str(p.relative_to(ROOT)) for p in stale))
            return 1
...
# ---- assets (SVG + Pillow) ------------------------------------------------------------------------------------------
def assets() -> int:
    from cleanbill import brand_assets
