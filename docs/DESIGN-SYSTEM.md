...
2. **Adding a colour** = a primitive in *both* theme files + a semantic alias in `tokens.css` + `python -m cleanbill.brand --sync`
...
- Text contrast ≥ 4.5:1; large text (≥ 24px, or ≥ 19px bold) and UI components ≥ 3:1 — checked by `python -m cleanbill.brand
...
**Letters** (`cleanbill/letters/generate.py`): US Letter, 0.75 in margins, first page only (Lob). The §41.0051(a) block is
...
**Packet data sheet and Form 50-114 audit page** (`cleanbill/agent/packet.py`, `cleanbill/agent/form50114.py`,
...
**E-mail** (`cleanbill/email/base.html`, `cleanbill.email.render_email(subject, body)`): a 600px table, every style inline, the
...
same body. Colours are inlined from `cleanbill/brand_tokens.py` at render time. Test in Gmail and Apple Mail after any change.

**Assets** (`apps/web/public/brand/`, regenerated by `python -m cleanbill.brand --assets`): `wordmark.svg` /
...
is `MARK` in `cleanbill/brand.py`; the web reads it from `brand.generated.ts`.
...
Generated by `python -m cleanbill.brand --contrast` (WCAG 2.2 relative luminance). Minimum 4.5 = body text, 3.0 = large text
