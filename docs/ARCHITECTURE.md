...
<td>Encodes the Tax Code rules: refundable years (§11.431 two-year look-back), per-unit exemption amounts (flat or percentage with $5,000 floor), OV65 stacking, conservative rounding. Input: appraised value, filing date, deed date → output: <code>RefundEstimate</code> (years, by-year savings by unit, total, forward annual, unconfirmed flag). <strong>Defect:</strong> rate table is fixed to five Austin units.</td>
...
<tr class="odd">
<td class="c">7</td>
<td>Static pages (→ Next.js on Vercel, T-12)</td>
<td>GitHub Pages · HTML/JS today</td>
<td class="mono">docs/index.html<br />
docs/claim.html<br />
docs/agreement.html<br />
docs/ops.html<br />
docs/config.js</td>
<td>Four pages, no build step, no framework. <code>config.js</code> holds the API base URL and brand. <code>claim.html</code> reads <code>?c=CODE</code>, calls the claim API, renders states (loading / not found / couldn't connect / already in progress / thanks), collects the form and posts it. <code>ops.html</code> asks for the password, stores it in memory, and renders the ops JSON. <strong>T-12:</strong> customer pages are rebuilt as a Next.js app on Vercel (SPEC-04) with inline validation (SPEC-06); the static pages remain as the fallback until cut-over.</td>
<td class="c">**Rebuild**</td>
</tr>
<tr class="even">
<td class="c">8</td>
<td>claim API</td>
<td>Supabase Edge Function · Deno/TS · v4</td>
<td class="mono">supabase/functions/claim/</td>
<td><strong>GET</strong> <code>?c=CODE</code>: normalises the code, rate-limits (120 views/IP/hour via <code>events</code>), loads lead + property, logs a <code>view</code> event, moves lead <code>new/mailed → opened</code>, returns estimate, situs, deadline. <strong>POST</strong> multipart: validates fields and consents, routes obvious ineligibility to <code>needs_review</code>, inserts <code>claims</code> (v2 name; <code>customers</code> in the prototype), uploads license image(s) to bucket <code>ids/</code>, inserts <code>documents</code>, marks lead <code>claimed</code>, logs event + audit, then asynchronously kicks module 9. CORS open (the code is the secret); <code>verify_jwt=false</code>.</td>
<td class="c">**Built**</td>
</tr>
<tr class="odd">
<td class="c">9</td>
...
|                          |                                                                                                                                                                                                                                                                                                                                                                                                                                         |
|--------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Supabase project         | Display name `texas-refund-desk` (renamed 2026-09-13; ref `letrfpwskjbgnyacesgv`, every URL unchanged). us-east-1, free tier. Functions deployed: claim v4, process-claim v3, ops v3, selftest v2 (all ACTIVE); bundles verified byte-identical to `supabase/functions/` on 2026-09-13. Per-function `verify_jwt` is recorded in `supabase/config.toml`.                                                                                                                                                                         |
| Public URLs              | `https://cponitz.github.io/texas-refund-desk/{index,claim,agreement,ops}.html` (GitHub Pages serves the `docs/` folder at the site root, so there is no `/docs/` path segment; the old `homestead-refund` address does not redirect). Target state (T-12): customer pages at `https://texasrefunddesk.com` on Vercel; `ops` at `https://ops.texasrefunddesk.com`. |
...
| Column                         | Type             | Definition                                                                                                                    | Set by |
|--------------------------------|------------------|-------------------------------------------------------------------------------------------------------------------------------|--------|
| prop_id / entity_cd            | bigint FK / text | Composite key. Entity code as TCAD's `PROP_ENT` file gives it (e.g. school district, city, county, ACC, Central Health, ESD). | 5      |
...
#### customers — v2 · the person / account (was: the engagement)
...
#### claims — v2 name for the prototype's `customers` · one engagement: person × property × service (1 row: the synthetic test)
...
| findings                                                                                    | jsonb — v2                              | **Replaces the concatenated `status_reason` string.** An array of structured findings: `[{code:"address_mismatch", severity:"blocking", field:"address", message:"…", detail:{id:"…", situs:"…"}}]`. Codes are an enum (`not_readable, not_texas_id, name_mismatch, signer_mismatch, low_confidence, under_18, address_mismatch, expired, over_65, not_primary, other_homestead, processing_error`). The page, the ops dashboard and the agent render from the code, not the sentence (SPEC-06). `status_reason` is kept as a generated display string during migration, then dropped. | 8, 9, 12          |
| created_at / updated_at                                                                     | timestamptz                             | Row timestamps.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | trigger           |
...
| kind                                                            | text                                 | `dl_front` (required for filing), `dl_back`, `other`, and — v2 — `typed_id`: the homeowner typed their license details instead of (or before) photographing it. **Why typed entry cannot replace the photo:** Tax Code §11.43(j) requires a *copy* of the license to accompany the application, so TCAD will not accept typed data alone. Typed entry is still valuable: it gives an instant address check before the photo step and a fallback when the photo is unreadable (SPEC-06). | 8         |
| storage_path / mime / bytes                                     | text / text / int                    | Path in bucket `ids/`, type, size (≤ 15 MB). Null for `typed_id`.                                                                                                                                                                                                                                                                                                                                                                                                                       | 8         |
...
|--------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|
| messages (claim_id, direction, channel, subject, body, intent, agent_draft, approved_by/\_at, sent_at) | Every communication, drafted or real. `direction` inbound\|outbound; `channel` email\|sms\|letter\|portal\|note; `intent` needs_dl_update \| ready_to_submit \| needs_review \| filed \| approved \| refund_notice \| denied \| reply \| other. `agent_draft=true` waits for a human; `sent_at` set by the Resend send (G-5). | 9, 12, 10, G-5 |
| events (at, claim_code, kind, detail)                                                                  | Funnel telemetry keyed by claim code. Kinds: `view, view_miss, claim_submitted`; SPEC-06 adds `validation_shown, dl_fix_started, card_saved, card_skipped` so the funnel can be read step by step.                                                                                                                            | 8, SPEC-06     |
...
<td>Array of <code>{code, severity: blocking|warning|info, field, message, detail}</code>. Rendering rules live in one place (<code>trd/findings.py</code> + <code>findings.ts</code> generated from the same table) so page, ops and agent say the same thing.</td>
</tr>
<tr class="even">
...
### 5.1 Lead pipeline (monthly, Mac)
    ...
    TCAD zip ─▶ load.py (layout spec → DuckDB appraisal_info, 493,324 rows)
            ─▶ leads.py: WHERE prop_type='R' AND state_cd IN (A1,A3,A4) AND no HS/OV65/OV65S/DP/DVHS flag
               ...
               estimate_refund(appraised_val, as_of, owned_since=deed_date) per lead; mint claim_code
            ─▶ leads.csv (22,889: 14,366 T1 · 2,409 T2 · 6,114 T3) ─▶ publish.py ─▶ Supabase (T1+T2 only, 16,775)
...
**Figure 5.1 — Claim submission sequence (as built).** Steps 1–14 are synchronous and fast; 15–19 run in the background after the page has already said thanks. **SPEC-06 changes step 14:** the new front-end waits for step 19 (polling the claim API every 2 s, ~8 s total) and shows the result on the same screen — "your license matches, here is your packet to review" or "the address on your license doesn't match; here is how to fix it" — instead of a generic thanks. The split between the two functions is unchanged (§3, panel).
...
**Findings (v2)** — structured validation results (`code, severity, field, message`) stored on the claim; prose is generated from them.
