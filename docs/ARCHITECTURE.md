...
**Ops API endpoints (SPEC-09 D1, ADR 0020).** All on the `ops` function; the ops password (`x-ops-key`) is the credential; 20 failed keys per IP per hour → 429. Every mutating action writes `audit_log` with `actor='ops'`; status guards are `ALLOWED_TRANSITIONS` (`logic.ts`, mirror of `trd/agent/store.py`) and answer 409 `<action>_not_allowed_from_<status>`.
...
| `POST /ops {action:"run_selftest", scenario}` | Server-side call of the `selftest` function with the same key. | `{ok, scenario, pass, ms, result}` (IPs masked) |
