...
def test_tools_happy_path_end_to_end_without_llm():
    ...
    claim = t.call("get_claim", {"customer_id": "cust-1"})
    assert claim["property"]["situs_full"].startswith("3675 DUVAL")
    assert claim["documents"][0]["extracted"]["dl_number"] == "***2728"  # masked
    v = t.call("validate_against_roll", {"customer_id": "cust-1"})
    assert v["status"] == "ready_to_submit"
    pk = t.call("generate_form_50114", {"customer_id": "cust-1", "over65": False})
    assert pk["bytes"] > 500 and len(pk["sha256"]) == 64
    assert t.call("set_status", {"customer_id": "cust-1", "status": "processing", "reason": "x"})["ok"]
    assert t.call("set_status", {"customer_id": "cust-1", "status": "ready_to_submit", "reason": "validated"})["ok"]
    d = t.call("draft_message", {"customer_id": "cust-1", "intent": "ready_to_submit", "subject": "Ready", "body": "Hi Richard, your packet is ready."})
    assert d["message_id"].startswith("msg-")


def test_tools_refuse_false_claims_in_drafts():
    t = Tools(FixtureStore(FIX), today=TODAY)
    d = t.call("draft_message", {"customer_id": "cust-1", "intent": "filed", "subject": "x", "body": "Good news, we have submitted your application."})
    assert "error" in d


def test_tools_block_illegal_transitions():
    t = Tools(FixtureStore(FIX), today=TODAY)
    r = t.call("set_status", {"customer_id": "cust-5", "status": "needs_dl_update", "reason": "nope"})  # ready_to_submit -> needs_dl_update not allowed
    assert "error" in r


def test_mismatch_case_routes_to_dl_update():
    t = Tools(FixtureStore(FIX), today=TODAY)
    v = t.call("validate_against_roll", {"customer_id": "cust-2"})
...
def test_loop_feeds_tool_results_back_and_stops_on_text():
    ...
    script = [
        [tu("a", "get_claim", {"customer_id": "cust-2"})],
        [tu("b", "validate_against_roll", {"customer_id": "cust-2"}), tu("c", "set_status", {"customer_id": "cust-2", "status": "processing", "reason": "start"})],
        [tu("d", "set_status", {"customer_id": "cust-2", "status": "needs_dl_update", "reason": "ID address 900 Congress Ave != situs"}),
         tu("e", "draft_message", {"customer_id": "cust-2", "intent": "needs_dl_update", "subject": "One quick step", "body": "Hi Richard, the address on your ID doesn't match. You can also file free at traviscad.org."})],
    ...
    assert store.data["customers"][1]["status"] == "needs_dl_update"


def test_loop_stops_at_max_turns():
    tools = Tools(FixtureStore(FIX), today=TODAY)
    client = FakeClient([[tu(str(i), "get_claim", {"customer_id": "cust-1"})] for i in range(10)])
