...
-- as events arrive. provider_message_id is unique (the constraint's index is the webhook's lookup path) and the second
-- half of the double-send guard (the update `… where sent_at is null` is the first). events.kind gains email_bounced /
-- email_complained (no constraint to change). Service role only, like every other table.
