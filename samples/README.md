...
Finished outputs of the prototype, checked in so they stay addressable. All synthetic — every file here uses the test lead `CB-TEST-0001` and the synthetic property account `999000001` (3675 DUVAL ST). No real homeowner data is in this folder. The prototype's `system-map.html` (26 old-brand references) was removed by SPEC-08; the Cowork project keeps the published system map.
...
The 25-lead verification pack is **not** here: it contains real owner names and addresses. It lives in `~/Desktop/ClaudeCowork/cleanbill/artifacts/`.
